"""
Plot pixel confusion matrices per model (fold_04), seen vs unseen side by side.
Produces one row per model — no aggregation across models.
"""
...
def load_cm(folder, split_prefix):
    """Load raw pixel confusion matrix from fold_04 of a single test folder."""
    fold_dirs = glob.glob(os.path.join(BASE, folder, "fold_04_*"))
    if not fold_dirs:
        print(f"[WARN] no fold_04 dir in {folder}")
        return None
    cm_path = os.path.join(fold_dirs[0], f"{split_prefix}_pixel_confusion_matrix.csv")
...
def plot_cm(ax, cm_raw, title):
    ...
    cm_norm = cm_raw / np.where(row_sums == 0, 1, row_sums)
    ...
    ax.set_xticklabels(CLASS_LABELS, rotation=35, ha="right", fontsize=10, fontweight="bold")
...
def main():
    ...
    fig, axes = plt.subplots(n_models, 2, figsize=(11, 6 * n_models))
    # fig.suptitle("Normalized Pixel Confusion Matrices (fold_04, per model)",
    ...
    plt.tight_layout()
    out_path = os.path.join(BASE, "confusion_matrices_plot.png")
