...
CONFIG = {
    ...
    # Week 4 U-Net patch-sampling training output folder from:
    #   week4_unet_patch_sampling_seen_unseen.py
    "model_output_subdir": r"Outputs/week4_train_unet_patch_sampling",
    "eval_output_subdir": r"Outputs/week4_test_unet_patch_sampling",

    # Expected checkpoint per fold:
    #   model_output_subdir/fold_name/checkpoints/best_unet_patch_sampling.pt
    "checkpoint_name": "best_unet_patch_sampling.pt",
    ...
    # ------------------------------------------------------------
    # Fallback U-Net architecture config.
...
def build_model(cfg: Dict):
    return smp.Unet(
        encoder_name=cfg_get(cfg, "encoder_name"),
        encoder_weights=None,  # checkpoint provides trained weights
        encoder_depth=int(cfg_get(cfg, "encoder_depth")),
        decoder_channels=tuple(cfg_get(cfg, "decoder_channels")),
        decoder_use_batchnorm=bool(cfg_get(cfg, "decoder_use_batchnorm")),
        decoder_attention_type=cfg_get(cfg, "decoder_attention_type"),
        in_channels=3,
        classes=int(cfg_get(cfg, "num_classes")),
        activation=cfg_get(cfg, "activation"),
    )
...
def evaluate_split(model, df: pd.DataFrame, cfg: Dict, device, split_name: str, fold_name: str, output_dir: Path):
    ...
    pd.DataFrame(
        [{"class_name": k, "IoU": v, "F1": pixel_metrics["per_class_f1"].get(k, np.nan)}
         for k, v in pixel_metrics["per_class_iou"].items()]
    ).to_csv(output_dir / f"{split_name}_pixel_per_class_metrics.csv", index=False)
...
def make_aggregate_tables(results_df: pd.DataFrame, damage_class_df: pd.DataFrame, output_root: Path):
    ...
    return agg, gap, gap_agg, damage_class_agg
...
def evaluate_one_fold(cfg: Dict, fold_dir: Path, fold_index: int):
    ...
    print_header(f"Week 4 U-Net patch-sampling full eval | {fold_dir.name}")
    ...
    try:
        model, model_cfg, ckpt_path = load_checkpoint_and_model(cfg, fold_dir, device)
...
def main():
    ...
    print_header("Week 4 U-Net patch-sampling full pixel + building evaluation plan")
    ...
    agg, gap, gap_agg, damage_class_agg = make_aggregate_tables(results_df, damage_class_df, output_root)
    ...
    print("  *_pixel_per_class_metrics.csv")
    ...
    print("  *_damage_class_metrics.csv")
