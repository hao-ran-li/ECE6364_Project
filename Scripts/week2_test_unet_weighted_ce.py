...
CONFIG = {
    ...
    "model_output_subdir": r"Outputs/week2_train_unet_weighted_ce",
    "eval_output_subdir": r"Outputs/week2_test_unet_weighted_ce",
...
def build_model(cfg: Dict):
    return smp.Unet(
        ...
        encoder_weights=None,  # trained weights come from checkpoint
...
def building_metrics_from_cm(cm: np.ndarray) -> Dict:
    ...
    recall = np.divide(tp, true, out=np.zeros_like(tp), where=true > 0)  # per-class building accuracy
    f1 = np.divide(
...
def update_building_cm_for_image(pred_mask: np.ndarray, row: pd.Series, building_cm: np.ndarray) -> int:
    ...
    # Original polygon coordinates are in original image pixel coordinates.
    orig_img = cv2.imread(str(image_path), cv2.IMREAD_COLOR)
    if orig_img is None:
        # xView2 images are usually 1024x1024; fallback only if image read fails.
    ...
    for feat in data.get("features", {}).get("xy", []):
        ...
        # For building-level damage classification, skip unclassified/background.
...
def evaluate_one_fold(cfg: Dict, fold_dir: Path, fold_index: int):
    ...
    print_header(f"U-Net full eval | {fold_dir.name}")
...
def main():
    ...
    print_header("U-Net full pixel + building evaluation plan")
