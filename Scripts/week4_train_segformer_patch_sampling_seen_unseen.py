...
CONFIG = {
    ...
    "output_subdir": r"Outputs/week4_train_segformer_patch_sampling",
    ...
    # ------------------------------------------------------------
    # SegFormer architecture configs
    # ------------------------------------------------------------
    "model_name": "SegFormer-B0",
    "model_source": "config",  # "config" or "pretrained"
    "use_pretrained": False,
    "pretrained_name": None,  # e.g. "nvidia/mit-b0" 
...
class SegFormerDenseWrapper(nn.Module):
    ...
    This wrapper upsamples logits to the input image size so the rest of
    the code can use:
        logits = model(images)
...
def build_model(cfg: Dict):
    if bool(cfg.get("use_pretrained", False)) or cfg.get("model_source") == "pretrained":
        try:
            base = SegformerForSemanticSegmentation.from_pretrained(
                str(cfg["pretrained_name"]),
                num_labels=int(cfg["num_classes"]),
                id2label=ID2LABEL,
                label2id=LABEL2ID,
            ...
            print("[WARN] Falling back to random-init SegFormer config.")

    model_cfg = build_segformer_config(cfg)
    base = SegformerForSemanticSegmentation(model_cfg)
    print_loaded_config(cfg, base.config)
    return SegFormerDenseWrapper(base)
...
@torch.no_grad()
def save_prediction_examples_generic(model, dataset, device, out_dir: Path, num_examples: int, prefix: str):
    """
    Local prediction-saving helper for patch-sampling extension.
    It assumes dataset returns dicts with image and mask.
    It works with SegFormerDenseWrapper because model(images) returns dense logits.
    """
    ...
    for k, idx in enumerate(indices):
        ...
        img_np = sample["image"].detach().cpu().numpy().transpose(1, 2, 0)
        # Try to visualize even if normalization differs slightly.
        img_np = img_np - img_np.min()
        img_np = img_np / max(img_np.max(), 1e-8)

        gt_color = palette[np.clip(mask, 0, 4)]
        pred_color = palette[np.clip(pred, 0, 4)]
        overlay = (0.65 * (img_np * 255) + 0.35 * pred_color).clip(0, 255).astype(np.uint8)
        ...
        image_id = str(sample.get("image_id", f"sample_{idx}"))
        fig.savefig(out_dir / f"{prefix}_{k:02d}_{image_id}.png", dpi=150, bbox_inches="tight")
        plt.close(fig)
        saved += 1
...
def evaluate_split(
    ...
    # Use unweighted CE for reporting loss only, metrics are from predictions.
...
def read_fold_data(fold_dir: Path, cfg: Dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    seen_test_df = pd.read_csv(seen_test_path)
    unseen_test_df = pd.read_csv(unseen_test_path)
    return train_df, val_df, seen_test_df, unseen_test_df
...
def run_one_fold(cfg: Dict, fold_dir: Path, fold_index: int):
    ...
    print_header(f"ECE 6364 - Week 4 SegFormer patch sampling | {fold_dir.name}")
    ...
    best_value = -1.0
    best_epoch = -1
    best_path = ckpt_dir / "best_segformer_patch_sampling.pt"
...
def main():
    ...
    print_header("SegFormer patch-sampling fold plan")
