...
CONFIG = {
    ...
    "model_output_subdir": r"Outputs/week3_train_segformer_weighted_ce",
    "eval_output_subdir": r"Outputs/week3_test_segformer_weighted_ce",

    # Expected checkpoint per fold:
    #   model_output_subdir/fold_name/checkpoints/best_segformer_weighted_ce.pt
    "checkpoint_name": "best_segformer_weighted_ce.pt",
...
def load_state_dict_flexible(model: nn.Module, state_dict: Dict):
    ...
    Supports checkpoints saved from:
      1. SegFormerDenseWrapper: keys start with wrapper prefix.
      2. Raw HuggingFace SegFormer: keys do not include the wrapper prefix.
    """
    try:
...
def evaluate_one_fold(cfg: Dict, fold_dir: Path, fold_index: int):
    ...
    print_header(f"SegFormer full eval | {fold_dir.name}")
...
def main():
    ...
    print_header("SegFormer full pixel + building evaluation plan")
